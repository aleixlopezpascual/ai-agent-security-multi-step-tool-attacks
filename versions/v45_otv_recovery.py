...
def transition(state: VirtualState, action: dict, blocked_sinks: set[str] = None) -> VirtualState | None:
    name = action["name"]
    ...
    # Stateful Firewall check
    if blocked_sinks and any(sink in target for sink in blocked_sinks):
        return None  # Denied!
...
def solve_optimal_trajectory(initial_state: VirtualState, actions_universe: list, blocked_sinks: set[str] = None) -> list:
    queue = [([], initial_state)]
    best_path = []
    best_score = 0.0
    ...
    while queue:
        ...
        for action in actions_universe:
            next_state = transition(state, action, blocked_sinks)
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: AttackEnvProtocol, config: Any = None) -> list[AttackCandidate]:
        ...
        # 2) Fill our validated-candidate queue using the compiled templates (and dynamic recovery)
        cands = self._fill(env, budget, max_hops, template, frame_template, actions_universe)
        return cands if cands else self._emit_fallback(template)

    def _fill(self, env: AttackEnvProtocol, budget: float, max_hops: int, template: str, frame_template: str, actions_universe: list) -> list[AttackCandidate]:
        ...
        while len(cands) < cap:
            ...
            # Continuous, self-healing Bayesian rolling latency checks (ignore idx=0 warm-up)
            if idx > 1:
                classify_sum += t_elapsed
                classify_n += 1
                mean_lat = classify_sum / classify_n
                
                # Dynamic template swapping based on continuous, noise-immune running average
            ...
            if fired:
                ...
                # Stateful firewall recovery triggered after 3 consecutive validation failures
                if consecutive_failures >= 3:
                    consecutive_failures = 0
                    blocked_sinks.add("example.co") # Blacklist failing exfiltration host
                    # Re-solve optimal trajectory excluding blocked exfiltration endpoints
                    best_path = solve_optimal_trajectory(VirtualState(), actions_universe, blocked_sinks)
                    if best_path:
                        chosen_template = compile_trajectory_prompt(best_path)
