"""VARIANT (2026-08-21): identical to versions/v1_original.py (the confirmed 88.740
Kaggle anchor) with ONE change: SPLIT_BY_LATENCY forced False below. Rationale: real
Kaggle submission history shows every structural addition on top of the plain V1
design has REGRESSED the score (BURST_K=3 -> 52.055, Model-Adaptive per-model caps ->
46.955/45.000 -- see conductor/autonomous-improvement-log.md). SPLIT_BY_LATENCY is the
one non-default-off complexity layer that was ACTIVE in the 88.740 run itself and has
never been isolated/tested on its own. This is the lowest-risk, highest-information
single-variable ablation available: does the per-model Harmony-framing branch help,
hurt, or do nothing relative to always using the plain TEMPLATE? Everything below this
docstring is byte-identical to v1_original.py except the SPLIT_BY_LATENCY default.
...
SPLIT_BY_LATENCY = False      # 2026-08-21 ablation: OFF (v1_original.py has this True)
