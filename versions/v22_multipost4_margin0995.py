"""v22 (2026-08-24): combines the TWO best individually-confirmed real Kaggle
results this session -- `v16_slow_multipost_n4.py`'s `SLOW_MULTIPOST_N=4`
(confirmed sweep peak, 87.815) and `v20_tighter_margins_0995.py`'s
`REPLAY_SAFE_FRAC=0.995` (NEW BEST SCORE, 90.135 -- first submission all
session to beat the 88.740 anchor). `v21_combined_margins_multipost.py`
already tested N=4 + REPLAY_SAFE_FRAC=0.99 (weaker margin, still PENDING at
time of writing); this uses v20's now-proven-superior 0.995 value instead.
`SPLIT_THRESHOLD_S`, `FILL_BUDGET_FRAC`, `BURST_K`, and every other knob are
untouched from v1_original.py.

Risk check (same as v21's, still applies): `REPLAY_SAFE_SIZING`'s fill loop
sizes its safety cushion from OBSERVED per-candidate latency generically, not
specific to N=1 vs N=4 multipost -- v20 already independently proved 0.995 is
safe (no void, real COMPLETE score), and v16 already independently proved
N=4 is safe. No new mechanism-specific risk from combining them, but this is
the first live test of that combination at THIS margin value.
...
# v22: v20's real-proven-BEST 0.995 (was v16/v1's 0.98) -- combined with SLOW_MULTIPOST_N=4 below.
