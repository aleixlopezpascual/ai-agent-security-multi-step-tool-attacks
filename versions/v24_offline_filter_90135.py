"""v24 (2026-08-25): v20_tighter_margins_0995.py's exact proven code with ONE further
architectural addition: `OfflineGuardrailFilter` pre-screening integrated inside the
`_fill` generation loop. This statically falsifies failing candidate prompts, saving
massive sandbox interaction latency and bypassing local evaluation saturation bounds.

Rationale: v20_tighter_margins_0995.py (REPLAY_SAFE_FRAC=0.995) resolved real on
Kaggle at 90.135 (submission 55728233, COMPLETE, 2026-08-24) -- our absolute best
leaderboard score. This version copies that exact top-scoring code and adds our
deterministic offline transition dynamics filter to boost candidate generation throughput.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(
        ...
        while len(cands) < cap:
            ...
            # Pre-filter using offline transition dynamics
            from versions.offline_filter import OfflineGuardrailFilter
            if not OfflineGuardrailFilter.verify_candidate(msg):
                continue
