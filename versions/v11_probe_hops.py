...
VARIANT (2026-08-22, EXPERIMENTAL, not yet real-submission-tested): activates the
never-before-used PROBE_HOPS lever documented (but disabled) in v1_original.py.
See memory probe-hops-calibration-result for the full writeup; summary below.

Live-measured (sibling worktree, real GGUF weights, 120s local fill budget,
`_fired()` check unaffected -- same 100% fire rate at hops=1 as hops=8 on both
models) per-candidate fill-loop latency, PROBE_HOPS=8 (today's live default) vs
PROBE_HOPS=1:

    Gemma:    1.498s -> 1.171s  (1.28x speedup)
    GPT-OSS:  3.515s -> 1.782s  (1.97x speedup)

The speedup is genuinely model-specific (GPT-OSS's per-hop wrap-up generation is
far more expensive, so skipping hops 1-7 saves more there), which matters because
REPLAY_COST_COEF is applied UNIFORMLY across both models (no per-model branching
-- that pattern already regressed hard in v6/v7, see versions/README.md). A
coefficient calibrated only to Gemma's 1.28x would undercharge GPT-OSS's true
replay-equivalent cost by nearly 2x and risk a REPLAY_SAFE_SIZING overrun (voids
the WHOLE submission per the file's own docs, not just one row). REPLAY_COST_COEF
is therefore set to 2.1 here: the worse-case (GPT-OSS) ratio of ~1.97x plus a
margin for the extra variance observed in the hops=1 timing samples (spread was
~4x wider than hops=8's), rather than either model's raw measured ratio.

STATUS: local-timing-calibrated only. Not yet locally smoke-tested end-to-end at
scale, and not yet real-submission-tested. Every other structural addition on top
of plain K=1 v1 has regressed on real Kaggle (6-for-6, see versions/README.md) --
treat this the same way: promising local signal is not evidence of a real win.
"""
...
PROBE_HOPS = 1                # 2026-08-22: activates the hops=1 fill-throughput lever
REPLAY_COST_COEF = 2.1        # calibrated to the worse-case (GPT-OSS ~1.97x) ratio + margin,
                               # not Gemma's 1.28x -- see memory probe-hops-calibration-result
