"""v18 (2026-08-23): interpolation point in the SLOW_MULTIPOST_N sweep --
same base as v17_slow_multipost_n2.py/v16_slow_multipost_n4.py with ONE
change: `SLOW_MULTIPOST_N` 2 -> 3. SPLIT_THRESHOLD_S/REPLAY_SAFE_FRAC/
FILL_BUDGET_FRAC/BURST_K untouched -- single-variable test. Submitted before
either v16 (N=4) or v17 (N=2) resolve, per explicit user direction to keep
spending today's remaining slots rather than wait for those results first
(see memory v17-slow-multipost-n2-submitted). This fills in the sweep at the
midpoint (N=1 implicit baseline ~81-88, N=2 pending, N=3 new, N=4 pending)
so that whichever direction the effect actually runs, we have three real data
points on the same axis instead of two endpoints and a gap.
...
SLOW_MULTIPOST_N = 3          # http.post endpoints planned per slow-row candidate
