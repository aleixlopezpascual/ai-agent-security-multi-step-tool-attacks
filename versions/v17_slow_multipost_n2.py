"""v17 (2026-08-23): v16_slow_multipost_n4.py's exact proven code with ONE
change -- `SLOW_MULTIPOST_N` 4 -> 2. Same `_forge_plan_msg` Harmony-forge
mechanism on the row `SPLIT_BY_LATENCY` classifies as "slow" (real GPT-OSS),
SPLIT_THRESHOLD_S/REPLAY_SAFE_FRAC/FILL_BUDGET_FRAC/BURST_K untouched -- this
is a single-variable test of a LOWER multipost count than v16, submitted
BEFORE v16's real score is known (user explicitly chose to spend a slot on
this now rather than wait; see memory slow-multipost-n-first-positive-lever
/ v16-slow-multipost-submitted-2026-08-23). Rationale for N=2 specifically:
the code's own historical probe-comment noted N=4's gain is "modest (~1.1x
throughput: 4x posts cost ~3.6x replay, each post is a full reasoning
generation)" -- i.e. replay cost scales close to linearly with N while the
GPT-OSS fire-rate cushion the loop relies on (REPLAY_SAFE_SIZING) shrinks
proportionally, so a smaller N trades some of the extra per-candidate raw for
a larger kept-candidate count. Whether N=2's lower per-candidate value but
higher volume beats N=4's higher per-candidate value but lower volume is an
open, untested question this submission answers with real data -- not derivable
from the local-only tests already run (those only tested N=1 vs N=4).
...
# v17 (2026-08-23): N lowered to 2 -- see module docstring for rationale.
