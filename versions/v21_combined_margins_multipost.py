"""v21 (2026-08-24): combines the TWO structural changes this session has now
independently confirmed NON-REGRESSIVE on real Kaggle -- `v12_tight_margins.py`'s
`REPLAY_SAFE_FRAC` 0.98 -> 0.99 (resolved 88.290, no fill-phase-overrun void) and
`v16_slow_multipost_n4.py`'s `SLOW_MULTIPOST_N` 1 -> 4 (resolved 87.815, confirms
the locally-repeated +5.9% raw/sec finding transfers to real Kaggle). Both are
real, resolved, non-regressive data points -- not two blind guesses stacked
together. `SPLIT_THRESHOLD_S`, `FILL_BUDGET_FRAC`, `BURST_K`, and every other
knob are untouched from v1_original.py.

Risk check before combining: `REPLAY_SAFE_SIZING`'s fill loop sizes its safety
cushion from the OBSERVED per-candidate latency during that specific run (see
`k1-variance-mechanism-backend-throughput-2026-08-24` memory) -- this mechanism
is generic to whatever design produces that latency, not specific to N=1 vs
N=4 multipost. No mechanism-specific reason combining a proven-safe margin with
a proven-safe (and real-positive) multipost value should introduce a NEW risk
beyond what each already tests independently, but this IS the first live test
of that assumption -- treat the resolved score as real evidence either way.

--- (original v16 docstring below, retained for provenance) ---

v16 (2026-08-23): v1_original.py's exact proven code with ONE change --
`SLOW_MULTIPOST_N` 1 -> 4. This activates the already-built, previously-dormant
`_forge_plan_msg` Harmony-forge lever on the row `SPLIT_BY_LATENCY` classifies
as "slow" (real GPT-OSS, whose real per-candidate latency ~20.4s already sits
well above the default SPLIT_THRESHOLD_S=12.0 -- left UNCHANGED here, since that
threshold is only mis-observed on this fast local Mac, not on real Kaggle
hardware; see memory slow-multipost-n-first-positive-lever /
k1-ceiling-decision-2026-08-23). SPLIT_THRESHOLD_S, FILL_BUDGET_FRAC, BURST_K,
and every other knob are untouched from v1_original.py.

Local validation (SPLIT_THRESHOLD_S forced to 0.5 ONLY to make GPT-OSS's
already-proven-slow classification observable on this fast local hardware,
NOT changed here): two independent 300s runs each of the N=4 test vs an N=1
control both landed the test at a byte-identical 6.85 raw/sec (60 kept
findings, 3576 raw) against the control's 6.60 then 6.33 raw/sec -- a
reproducible +4-8% delta, not single-run noise. This is the first structural
change all session to come back net POSITIVE (not just less-negative) on a
repeat run; every prior structural gamble (BURST_K, PROBE_HOPS, multi-turn,
CONFUSED_DEPUTY stacking, same-turn combos) regressed. Distinct from those
because `_forge_plan_msg` uses a token-level Harmony channel forge (near-100%
compliance per the code's own historical probe comment below), not a
natural-language "call K times" instruction the model complies with
unreliably -- and it only ever touches the row already paying the
FRAME_TEMPLATE latency tax (never Gemma, which stays under threshold on real
hardware).
...
# v21: v12's proven-safe 0.99 (was v16/v1's 0.98) -- combined with SLOW_MULTIPOST_N=4 below.
