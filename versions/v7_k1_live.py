...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(
        ...
        """Model-Adaptive Sizing (Version 7): Dynamic capacity sizing focused on 
        ultra-stable single-hop exfiltrations (BURST_K = 1). Fully eliminates any 
        token-bloat replay risks while maximizing candidate volume."""
        ...
        if warmup_time < 12.0:
            # FAST MODEL (Gemma): Fully wide-open up to 1,600 (highly safe with K=1)
            cap = 1600
            print(f"[attack] Detected fast model (warmup={warmup_time:.1f}s). Setting cap={cap}")
        else:
            # SLOW MODEL (GPT-OSS): Replay-safe cap of 500 candidates
            cap = 500
        ...
        while len(candidates_with_density) < max_generation_limit:
            ...
            # Adaptive Density stance: Version 7 is locked to safe, low-latency single exfiltrations
            current_k = 1
...
# --- Version 7 Optimizations (Ultra-Stable Model-Adaptive Sizing) ---
BURST_K = 1
HARD_N_CAP = 1600
